...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        # 又例如，根据注意力公式得到的num_heads个(src_len, d_v)矩阵Z_i，并将其拼接起来，得到一个
    ...
    def forward(self, q_input, k_input, v_input, attn_mask=None):
        """
        :param q_input:   (batch_size, q_len, embed_dim)的张量
        :param k_input:   (batch_size, k_len, embed_dim)的张量
        :param v_input:   (batch_size, v_len, embed_dim)的张量，其中v_len=k_len
        ...
        residual = q_input  # 用于残差连接，为(batch_size, q_len, embed_dim)的张量
        ...
        q, k, v = [W(X).view(batch_size, -1, self.num_heads, self.d_k).transpose(1, 2)
                   for W, X in zip(self.linear_lst, (q_input, k_input, v_input))]
